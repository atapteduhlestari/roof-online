...
@endpush
@section('title', 'GA | Edit Storage Transaction')
@section('container')
    <div class="container-fluid">
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Transaction | Edit Storage</h1>
        <div class="mb-5">
            <form action="/trn-storage/{{ $trnStorage->id }}" method="POST" id="formTrnstorage">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('trn_date') is-invalid @enderror" name="trn_date"
                            value="{{ old('trn_date', $trnStorage->trn_date) }}">
                    </div>
                </div>
                <hr>
                <div class="row">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control @error('pelaksana') is-invalid @enderror" id="select-child"
                            name="pelaksana">
                            <option value="">Pelaksana</option>
                            @foreach ($employees as $pelaksana)
                                <option value="{{ $pelaksana->name }}"
                                    {{ old('pelaksana', $trnStorage->pelaksana) == $pelaksana->name ? 'selected' : '' }}>
                        ...
                        <select class="form-control @error('penyetuju') is-invalid @enderror" name="penyetuju">
                            ...
                            @foreach ($employees as $penyetuju)
                                <option value="{{ $penyetuju->name }}"
                                    {{ old('penyetuju', $trnStorage->penyetuju) == $penyetuju->name ? 'selected' : '' }}>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-around">
                                                <div>
                                                    <a title="Detail Data" href="/trn-storage/{{ $trn->id }}"
                                                        class="btn btn-outline-dark text-xs">
                                                        <i class="fas fa-search-plus"></i>
                                                    </a>
                                                </div>
                                                <div>
                                                    <a title="Edit Data" href="/trn-storage/{{ $trn->id }}/edit"
